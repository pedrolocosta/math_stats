...
nivel_stdk = pd.DataFrame({'grau':[  80,   81,   82,   83,   84,   85,   86,   87,   88,   89,   90,   91,   92,   93,   94,   95,   96,   97,   98,   99],
                           ...
                           'za_2':[1.28, 1.31, 1.34, 1.37, 1.40, 1.44, 1.47, 1.51, 1.55, 1.59, 1.64, 1.69, 1.75, 1.81, 1.88, 1.96, 2.05, 2.17, 2.33, 2.57]})
...
class Size_mean():
    def __init__(self, tl, me, pstd, ps=0):
        ...
        self.pstd = pstd                # population standard deviation - média populacional        
        ...
        if self.ps == 0:
            ...
            self.ss = ceil((self.z_eq*self.pstd/self.me)**2)
        ...
        # if the population is finit - se a população for finita
            ...
            self.ss = ceil((self.ps*(self.pstd**2)*(self.z_eq**2))/(((self.ps-1)*(self.me**2))+((self.pstd**2)*(self.z_eq**2))))
        return
                   
    # summary of everything - resumo de tudo
    def describe(self):
        if self.ps == 0:
            print('population standard deviation - média populacional = {}'.format(self.pstd))
