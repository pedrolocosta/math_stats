...
class Size_sample():
    def __init__(self, tl, me, sp=50, ps=0):
        self.tl = tl                    # trust level - nível de confiança
        self.me = me/100                # margin of error - margem de erro
        self.sp = sp/100                # sample proportion - proporção amostral        
        ...
        if self.ps == 0:
            ...
            self.ss = ceil(((self.z_eq**2)*(self.sp*(1-self.sp)))/(self.me**2))
        ...
        # if the population is finit - se a população for finita
            ...
            self.ss = ceil((self.ps*self.sp*(1-self.sp)*(self.z_eq**2))/((self.sp*(1-self.sp)*(self.z_eq**2))+((self.ps-1)*(self.me**2))))
        return
    ...
    def describe(self):
        if self.ps == 0:
            ...
            print('margin of error - margem de erro = {:.2f}'.format(self.me))
            print('sample proportion - proporção amostral = {}'.format(self.sp))
